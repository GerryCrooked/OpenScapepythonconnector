...
class Associate_source_diagnostic(univ.Choice):
  componentType=namedtype.NamedTypes(
    namedtype.NamedType('asce-service-user',univ.Integer().subtype(implicitTag=tag.Tag(tag.tagClassContext,tag.tagFormatConstructed,1))),
    namedtype.NamedType('asce-service-provider',univ.Integer().subtype(implicitTag=tag.Tag(tag.tagClassContext,tag.tagFormatConstructed,2)))
    )
...
class AARE_apdu(univ.Any):
  tagSet = univ.Any.tagSet.tagImplicitly(tag.Tag(tag.tagClassApplication,tag.tagFormatConstructed,1))
